  ...
  function (MessageBox, MessageToast, BusyIndicator, Util) {
    ...
    return {
      getDataRolesxElemento: function (iIDRol, sNameApp, sIdElement, oContext) {
        ...
        let sql =
          'SELECT Roles."nombre_rol" , Roles."estado" , TypeElemen."ID_elemento", TypeElemen."tipo_elemento", RXE."read", RXE."update" ' +
          'FROM <tenant_schema>."ROLES_X_ELEMENTOS" AS RXE INNER JOIN <tenant_schema>."TIPOS_ELEMENTOS" AS TypeElemen ' +
          'ON TypeElemen."id" = RXE."ID_elemento"  INNER JOIN <tenant_schema>."MODULO" AS Modulos ' +
          'ON Modulos."id" = TypeElemen."ID_Modulo" INNER JOIN <tenant_schema>."ROLES" AS Roles ' +
          'ON Roles."id" = RXE."ID_rol" WHERE Roles."id" =' +
          iIDRol +
          ' AND Roles."estado"= true AND  Modulos."nombre_tecnico"=\'' +
          sNameApp +
          "' " +
          addSQL +
          ...
          success: function (data) {
            if (data) {
              ...
              for (let i = 0; i < arrResult.length; i++) {
                ...
                if (bExistsElement) {
                  let bHasEnabled =
                    bExistsElement && oElement.getEnabled !== undefined;
                  let objRolesxElement = {};
                  objRolesxElement.nombre_rol = arrResult[i].ID;
                  objRolesxElement.estado = that.defaultBoolean(
                    arrResult[i].NAME
                  );
                  objRolesxElement.id_elemento = arrResult[i].DESC1;
                  objRolesxElement.toHaveEnebled = bHasEnabled;
                  objRolesxElement.read = that.defaultBoolean(
                    arrResult[i].DESC3
                  );
                  objRolesxElement.update = that.defaultBoolean(
                    arrResult[i].DESC4
                  );
      ...
      getRol: function (sEmailUser) {
        let iIdRol = null;
        let sql =
          "SELECT * FROM <tenant_schema>.USUARIOS " +
          'WHERE "ID_Usuario"=\'' +
          sEmailUser +
          "' ;";
        let oConsulData = {
